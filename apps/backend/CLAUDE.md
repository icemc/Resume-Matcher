...
Every language is its own **tenant** (see [i18n.md](../../docs/agent/features/i18n.md)). `_tenant.py`'s `validate_tenant_language(language)` (a plain function, not `Depends()` — the codebase has zero DI precedent) 400s an unsupported language. List/create/upload endpoints take `language` as a **required** `Query(...)`/`Form(...)`/body field; by-id endpoints take it as an **optional** `Query(None)` for a soft 404-on-mismatch (never 403, to avoid leaking existence).

- `health.py` — `GET /health` (liveness, no LLM call), `GET /status` (LLM health + DB stats).
- `config.py` — `/config/llm-api-key` (GET/PUT), `/config/llm-test` (POST live health check), `/config/features`, `/config/language` (vestigial — `content_language` is only used as the migration backfill default now), `/config/prompts`, `/config/feature-prompts`, `/config/api-keys` (per-provider CRUD), `/config/reset` (POST; confirmation token `{"confirm": "RESET_ALL_DATA"}` in the JSON **body**, not a query param).
- `resumes.py` — the biggest router: `/resumes/upload` (`language: Form`, required), `GET /resumes` (`language: Query`, required — 404 on cross-tenant mismatch), `/resumes/list` (`language: Query`, required), `/resumes/configured-languages` (NOT tenant-scoped — lists tenants with ≥1 resume, feeds the frontend tenant switcher), `/resumes/improve` + `/improve/preview` + `/improve/confirm` (no language param — resolves `output_language` from the loaded resume's own `language` column), `PATCH /resumes/{id}`, `/{id}/pdf`, `/{id}/retry-processing`, cover-letter/outreach/title PATCH + on-demand generate, `/{id}/job-description`, `/{id}/cover-letter/pdf`.
- `jobs.py` — `/jobs/upload` (`JobUploadRequest.language`, required — batch JD text → job_ids), `GET /jobs/{id}` (optional soft tenant check).
- `enrichment.py` — `/enrichment/analyze/{id}`, `/enhance`, `/apply/{id}`, `/regenerate`, `/apply-regenerated/{id}` — no language param, resolves `output_language` from the resume.
- `applications.py` — the Kanban tracker: `GET /applications` (`language: Query`, required — board scoped to one tenant), `POST /applications` (`ManualApplicationCreate.language`, required), by-id/bulk endpoints (`GET/PATCH/DELETE /applications/{id}`, `/bulk`, `/bulk-delete`) take no language param (operate on explicit id lists).
- `resume_wizard.py` — `/resume-wizard/turn` (adaptive Q&A, no language param), `/resume-wizard/finalize` (`language`, required — creates the master resume for that tenant, `409` if one already exists).
...
- **Master resume invariant:** exactly one resume has `is_master=True` **per tenant (language)** — a composite `(language, is_master)` partial unique index, not a single global one. Concurrent uploads use `create_resume_atomic_master` (a single global `asyncio.Lock`, not sharded per tenant — master promotion is rare/human-paced, so cross-tenant serialization costs nothing meaningful) and auto-promote if the current tenant's master is stuck `failed`/`processing`.
...
| Dir | What | Notes |
|-----|------|-------|
| `unit/` | pure functions | diffs, `llm` provider/key helpers, parser date-restore, real-SQLite CRUD, `test_migrate_add_language_column.py` (idempotent schema migration + backfill + index swap) |
| `service/` | service layer, **LLM mocked** | improver diff flow, prompt construction |
| `integration/` | endpoints via httpx `ASGITransport` | config/health/jobs/resume/upload, plus `test_llm_contract.py` (real `llm.py` over `respx`), `test_pipeline_e2e.py` (upload→tailor→render, real routers + real temp DB), `test_tenant_isolation.py` (cross-tenant data isolation, concurrent-upload master race, unsupported-language 400s), `test_configured_languages.py` |
